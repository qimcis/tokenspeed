...
`_hopper_block32_policy.py` for validated exact H20 and H20-3e shapes with `[1, 32]` blocks,
...
A separate H20-3e TP8/EP8 AIPerf run compared this standalone branch against
the compatible base runtime with the same InstantTensor loader overlay. Mean
single-request TPOT fell from 23.82 to 17.30 ms at 1K/512 and from 24.46 to
17.93 ms at 16K/512. Output throughput rose from 41.11 to 56.20 tokens/s and
from 30.88 to 38.34 tokens/s, respectively. At 8K/128 concurrency 4 and
8K/512 concurrency 8, output throughput rose 2.2% and 5.4%, while mean TTFT
rose 3.6% in both cases. All 112 measured requests per arm had matching exact
input and output lengths. This was one ordered A/B run on synthetic prompts;
it does not establish statistical significance or a prefill speedup.
